...
unsigned char idata *Src_Pointer;//retrieve pointer dataa
...
unsigned char i,j;
	//top line
	//Instruction_Setting('I',0x00,0x00);//Set_Start_Page(0x00);
	//Instruction_Setting('A',XLevel,0x00);//Set_Start_Column(XLevel);
	...
	//button line
	//Instruction_Setting('I',0x07,0x00);//Set_Start_Page(0x07);
	//Instruction_Setting('A',XLevelL,0x00);//Set_Start_Column(XLevel);
...
unsigned int i;	
	...
	{
		//Instruction_Setting('D',i,0x00);//Set_Contrast_Control(i);
...
//  Initialization
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//  set display 	 Default => 0xAE ,  0xAE => Display Off  ,  0xAF => Display On
void OLED_Init()
{
unsigned char i;
...
//void   	ByteWrite(Uint8 src);
//Uint8  	ByteRead(void);
Uint16 	Thermal_detecting(void);
Uint16 	MaxRead(unsigned char select);
	...
	while(1)
	{
		...
		*/
		...
		for(r=0;r<10;r++){
			...
			//printf("T1:0x%x,T2:0x%x,T3:0x%x\n",T1,T2,T3);
...
Uint16 Thermal_detecting(void){
	  ...
	  float test=0x0000;
		...
		qq=qq&0xE6;//P24=1,P23=1,P20=1
		qq=qq+0x19;
		P2=qq;//P24=1,P23=1,P20=1
		_nop_();
    ...
    Uint16 i;
	  ...
	  Uint16 tmp;
		...
		if(select=='1'){
			...
			qq=qq&0xE6;//P24=1,P23=0,P20=0
			qq=qq+0x10;			
		}
		else{
		}
		
		P2=qq;//P24=0,P23=0,P20=1
